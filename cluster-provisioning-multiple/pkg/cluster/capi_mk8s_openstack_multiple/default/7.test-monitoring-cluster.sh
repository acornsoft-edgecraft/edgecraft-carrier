...
if [ $list_cnt -eq $total_cluster ]; then
    ...
    do
        ...
        get_start_time=`kubectl --kubeconfig=./kubeconfig describe machines.cluster.x-k8s.io  $i-control-plane | grep "Creation Timestamp" | awk '{print $3}'`
        ...
        echo "----- $name -----------------------------------------------------------------------------------------------------------------------------------------------------------" >> $log_path
        echo "Duration of time: $(TZ='Asia/Seoul' date -d @${duration_of_time} '+%H:%M:%S')" >> $log_path
    ...
    if [ $total_instance -eq $(($control_plane + $md)) ]; then
        ...
        if [ -z "$result" ]; then
            ...
            total_duration=$(TZ='Asia/Seoul' date -d @${sec_total_duration} "+%Y-%m-%d(%A) %H:%M:%S")
