...
- hosts 파일 적용
```sh
## hosts 파일에 node IP와 externalURL의 도메인 값을 추가 한다.
$ sudo vi /etc/hosts
192.168.88.181  core.harbor.domain
```

- docker 설정에 harbor registry 인증서 적용
```sh
## macOS 일경우 인증서를 다운받아 ~/.docker/certs.d/ 디렉토리에 추가 한다.
## 설정 후 docker 재시작 한다
$ curl -Lk https://192.168.88.181:32443/api/v2.0/systeminfo/getcert -o ~/.docker/certs.d/core.harbor.domain:32443/ca.crt
```

- 확인
```bash
## step-1. docker login 확인
...
### docker push
$ docker tag coredns/coredns:1.9.3 core.harbor.domain:32443/edgecraft/coredns/coredns:1.9.3
$ docker push core.harbor.domain:32443/edgecraft/coredns/coredns:1.9.3
df1818f16337: Pushed
256bc5c338a6: Pushed
1.9.3: digest: sha256:bdb36ee882c13135669cfc2bb91c808a33926ad1a411fee07bd2dc344bb8f782 size: 739

## step-2. helm login 확인
### helm version 1.12.0 이상
$ helm registry login core.harbor.domain:32443 --ca-file ~/.docker/certs.d/core.harbor.domain:32443/ca.crt
Username: admin
Password:
Login Succeeded

### helm chart push
$ helm push harbor-1.12.2.tgz oci://core.harbor.domain:32443/edgecraft --ca-file ~/.docker/certs.d/core.harbor.domain:32443/ca.crt
Pushed: core.harbor.domain:32443/edgecraft/harbor:1.12.2
Digest: sha256:f7b5df1e4c2ee7a759a8d76051c5ce9df307bd250631c2a1470217690e0d6bc4
```